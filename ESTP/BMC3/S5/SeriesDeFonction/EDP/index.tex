...
On dit que $F$ est \og  une intégrale première de $(E)$ \fg{} .
\end{proposition}
...
$$
\frac{\partial F}{\partial X} = -2 X^{2} + 3 XY - Y^{2} \Longrightarrow F(X, Y) = -\frac{2}{3} X^{3} + \frac{3}{2} X^{2} Y - XY^{2} + K(Y)
$$
Finalement, on trouve $f(x, y) = -\frac{2}{3}(x+y)^{3} + \frac{3}{2}(x+y)^{2}(x+2y) -(x+y)(x+2y)^{2} + K(x+2y)$ où $K$ est une fonction de classe $\mathcal{C}^{1}$.
...
\begin{remarque}
Cas d'une \og  vitesse constante \fg{} .
\end{remarque}
...
\begin{remarque}
Cas d'une \og  vitesse non constante \fg{} .
\end{remarque}
