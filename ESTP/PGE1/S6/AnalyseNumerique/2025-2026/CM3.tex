...
% ====================================================================
\section*{7. Bilan --- ce qu'on emporte pour l'examen blanc}
% ====================================================================

\atrou[5.5cm]{%
\begin{enumerate}[leftmargin=*]
  \item \textbf{Problème inverse :}
        on a des mesures, on veut un paramètre
        $\to$ poser $g(k) = 0$.
  \item \textbf{Méthode de Newton :}
        on linéarise et on itère,
        $x_{n+1} = x_n - g(x_n)/g'(x_n)$.
        Convergence quadratique,
        mais dépendante du point de départ.
  \item \textbf{Convergence quadratique :}
        le nombre de chiffres corrects
        double à chaque itération.
  \item \textbf{Moindres carrés :}
        plusieurs mesures $\to$ fonction coût
        $J(k) = \sum(\text{résidus})^2$ ;
        minimiser $J \equiv$ annuler $J'$,
        que l'on traite à nouveau par Newton.
\end{enumerate}
}

\medskip

\textbf{Rôle dans le module.}
Le CM3 fait dialoguer tous les acquis :

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ll}
  \hline
  \textbf{Outil} & \textbf{Provenance} \\
  \hline
  EDO de refroidissement & CM1 \\
  Différences finies & CM2 (approche de $g'$ si besoin) \\
  Schémas pour évaluer $T_{\mathrm{pred}}(k)$ & TD4 \\
...
\textbf{Le CM4 sera un examen blanc.}
Format et difficulté proches de l'examen du 26~mai.
Il croisera système linéaire, EDO/EDP,
interpolation ou intégration numérique,
avec de courts raisonnements sur la stabilité
...
une EDO, un problème inverse, Newton,
et une ouverture vers les moindres carrés.\\
C'est exactement le type d'enchaînement
attendu à l'examen.
\end{mdframed}
\end{center}
