...
\title{Examen de rattrapage S5 - Mathématiques - Analyse et Algèbre }
...
\begin{center}
    ...
    \begin{tabular}{|>{\centering\arraybackslash}m{4cm}|>{\centering\arraybackslash}m{6cm}|>{\centering\arraybackslash}m{4cm}|}
        ...
        \textbf{Contrôle de connaissances et de compétences -- Rattrapage} & 
        \textbf{FO-002-VLA-XX-001} \\
        \hline
        \textbf{27/02/2026}  &  & \textbf{Page 1/3} \\
        \hline
    \end{tabular}
\end{center}
\vspace{1em}
...
\begin{center}
    ...
    \begin{tabular}{|c|m{10cm}|}
        \hline 
        \multicolumn{2}{|c|}{\textbf{ANNÉE SCOLAIRE 2025-2026 -- Semestre 1 -- Rattrapage}} \\
        \hline 
...
\[
\langle \mathbf{e}_1, \mathbf{e}_1 \rangle = 1, \quad \langle \mathbf{e}_1, \mathbf{e}_2 \rangle = -1, \quad \langle \mathbf{e}_1, \mathbf{e}_3 \rangle = 0
\]
\[
\langle \mathbf{e}_2, \mathbf{e}_2 \rangle = 2, \quad \langle \mathbf{e}_2, \mathbf{e}_3 \rangle = 1, \quad \langle \mathbf{e}_3, \mathbf{e}_3 \rangle = 3
\]
...
\begin{enumerate}
    ...
    \[
    (g_{ij}) = \begin{pmatrix} 1 & -1 & 0 \\ -1 & 2 & 1 \\ 0 & 1 & 3 \end{pmatrix}
    \]
    \end{solution}
    }{}
    
    \item Soit le vecteur $\mathbf{v} = (2, 1, -1)$ exprimé dans la base $\{\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$.
    ...
    \[
    v_1 = g_{11}v^1 + g_{12}v^2 + g_{13}v^3 = 1 \cdot 2 + (-1) \cdot 1 + 0 \cdot (-1) = 2 - 1 = 1
    \]
    \[
    v_2 = g_{21}v^1 + g_{22}v^2 + g_{23}v^3 = (-1) \cdot 2 + 2 \cdot 1 + 1 \cdot (-1) = -2 + 2 - 1 = -1
    \]
    \[
    v_3 = g_{31}v^1 + g_{32}v^2 + g_{33}v^3 = 0 \cdot 2 + 1 \cdot 1 + 3 \cdot (-1) = 1 - 3 = -2
    \]
    \end{solution}
    }{}
    ...
    \begin{solution}
    \[
    \|\mathbf{v}\|^2 = v^1 v_1 + v^2 v_2 + v^3 v_3 = 2 \cdot 1 + 1 \cdot (-1) + (-1) \cdot (-2) = 2 - 1 + 2 = 3
    \]
    Donc $\|\mathbf{v}\| = \sqrt{3}$.
    \end{solution}
    }{}
...
\[
f(x) = \frac{1}{\sqrt{x}}, \qquad g(x) = e^{2ix - 3x}, \qquad h(x) = \cos(x^3)
\]

\begin{enumerate}
    \item Déterminer pour chaque fonction si elle appartient à $L^1(]0, 1[)$. Si oui, calculer sa norme. \textit{(1.5 pts)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    \[
    \int_0^{1} \frac{1}{\sqrt{x}} \, dx = \left[ 2\sqrt{x} \right]_0^{1} = 2 - 0 = 2
    \]
    L'intégrale converge, donc $f \in L^1(]0, 1[)$ et $\|f\|_1 = 2$.
    \end{solution}
    }{}
    
    \item Déterminer pour chaque fonction si elle appartient à $L^2(\mathbb{R}^*_+)$. Si oui, calculer sa norme. \textit{(1.5 pts)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    \[
    \int_0^{+\infty} |e^{2ix - 3x}|^2 \, dx = \int_0^{+\infty} e^{-6x} \, dx = \left[ -\frac{e^{-6x}}{6} \right]_0^{+\infty} = \frac{1}{6}
    \]
    L'intégrale converge, donc $g \in L^2(\mathbb{R}^*_+)$ et $\|g\|_2 = \sqrt{1/6} = \dfrac{1}{\sqrt{6}}$.
    \end{solution}
    }{}
    
    \item Déterminer pour chaque fonction si elle appartient à $L^\infty(\mathbb{R})$. Si oui, donner sa norme. \textit{(1 pt)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    $|\cos(x^3)| \leq 1$ pour tout $x \in \mathbb{R}$, et cette borne est atteinte (par exemple en $x = 0$ où $\cos(0) = 1$).
...
\begin{enumerate}
    \item Calculer $\mathcal{L}\{e^{2t}\}(s)$ directement à partir de la définition et préciser le domaine de convergence. \textit{(1 pt)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    \[
    \mathcal{L}\{e^{2t}\}(s) = \int_0^{+\infty} e^{2t} e^{-st} \, dt = \int_0^{+\infty} e^{-(s-2)t} \, dt = \left[ -\frac{e^{-(s-2)t}}{s-2} \right]_0^{+\infty} = \frac{1}{s-2}
    \]
    Cette intégrale converge pour $\text{Re}(s) > 2$.
    \end{solution}
    }{}
    
    \item En utilisant la propriété de décalage en fréquence $\mathcal{L}\{e^{at}f(t)\}(s) = \mathcal{L}\{f(t)\}(s-a)$, calculer $\mathcal{L}\{t^3 e^{-2t}\}$. \textit{(1 pt)}
    
    \textit{On rappelle que $\mathcal{L}\{t^n\} = \frac{n!}{s^{n+1}}$.}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    On utilise le décalage avec $f(t) = t^3$ et $a = -2$.
    
    On a $\mathcal{L}\{t^3\} = \frac{3!}{s^4} = \frac{6}{s^4}$.
    ...
    \item Résoudre l'équation différentielle $y' + 3y = e^{-t}$ avec $y(0) = 0$ en utilisant la transformée de Laplace. \textit{(2 pts)}
    ...
    \]
    \begin{itemize}
        \item $s = -1$ : $1 = A(2)$, donc $A = \frac{1}{2}$
        \item $s = -3$ : $1 = B(-2)$, donc $B = -\frac{1}{2}$
    \end{itemize}
    
    Donc $Y(s) = \dfrac{1/2}{s+1} - \dfrac{1/2}{s+3}$.
    
    \textbf{Étape 3 : Transformation inverse.}
    \[
    y(t) = \frac{1}{2}\left(e^{-t} - e^{-3t}\right)
    \]
    \end{solution}
    }{}
\end{enumerate}
...
\begin{enumerate}
    \item Soit $f(x, y) = e^{3x} \cos(2y)$. Calculer les dérivées partielles d'ordre $2$ de $f$. \textit{(1 pt)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    \[
    \frac{\partial f}{\partial x} = 3e^{3x}\cos(2y), \qquad \frac{\partial f}{\partial y} = -2e^{3x}\sin(2y)
    \]
    \[
    \frac{\partial^2 f}{\partial x^2} = 9e^{3x}\cos(2y), \qquad \frac{\partial^2 f}{\partial y^2} = -4e^{3x}\cos(2y)
    \]
    \end{solution}
    }{}
    ...
    \begin{solution}
    \[
    \frac{\partial^2 f}{\partial x^2} + \frac{\partial^2 f}{\partial y^2} = 9e^{3x}\cos(2y) - 4e^{3x}\cos(2y) = 5e^{3x}\cos(2y) = 5f
    \]
    L'équation est bien vérifiée.
    \end{solution}
    }{}
    
    \item On considère l'équation de la chaleur $\frac{\partial u}{\partial t} = k \frac{\partial^2 u}{\partial x^2}$ sur $[0, L] \times \mathbb{R}_+$ avec conditions aux bords $u(0,t) = u(L,t) = 0$.
    ...
    Pour $T$, l'équation $T' + \mu_n^2 k \, T = 0$ a pour solution générale :
    \[
    T_n(t) = C_n \, e^{-\frac{n^2\pi^2 k}{L^2} \, t}
    \]
    \end{solution}
    }{}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Soit $f(x) = H(x-1) - H(x-4)$ (\og fonction porte \fg{} sur $[1,4]$). Calculer la dérivée de $f$ au sens des distributions. \textit{(1 pt)}
    
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    La fonction $f$ vaut $1$ sur $[1,4[$ et $0$ ailleurs.
    
    \begin{itemize}
        \item La dérivée classique est nulle partout où $f$ est continue.
        \item Saut en $x = 1$ : $\sigma_1 = 1 - 0 = 1$
        \item Saut en $x = 4$ : $\sigma_4 = 0 - 1 = -1$
    \end{itemize}
    
    Donc $f' = \delta_1 - \delta_4$.
    \end{solution}
    }{}
    ...
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
    On calcule d'abord le saut en $x = 2$ :
    \begin{itemize}
        \item Limite à gauche : $\lim_{x \to 2^-} g(x) = 6$
        \item Limite à droite : $\lim_{x \to 2^+} g(x) = 4$
        \item Saut : $\sigma_2 = 4 - 6 = -2$
    \end{itemize}
    ...
    Par la formule des sauts : $g' = \{g'\} + \sigma_2 \delta_2$
    
    \textbf{Donc :} $g'(x) = \{g'\}(x) - 2\delta_2$
    \end{solution}
    }{}
    ...
    \begin{itemize}
        ...
        \item Limite à droite : $\lim_{x \to 0^+} h(x) = 3 - 0 = 3$
        \item Saut : $\sigma_0 = 3 - 1 = 2$
    \end{itemize}
    
    \textbf{En $x = 1$ :}
    \begin{itemize}
        \item Limite à gauche : $\lim_{x \to 1^-} h(x) = 3 - 1 = 2$
        \item Limite à droite : $\lim_{x \to 1^+} h(x) = 4$
        \item Saut : $\sigma_1 = 4 - 2 = 2$
    \end{itemize}
    ...
    \textbf{Donc :} $h'(x) = \{h'\}(x) + 2\delta_0 + 2\delta_1$

    Donc 
    $$\langle h', \varphi \rangle = \int_{-\infty}^{+\infty} \{h'\}(x) \varphi(x) \, dx + 2\varphi(0) + 2\varphi(1) = 
    \int_{-\infty}^{0} 2e^{2x} \varphi(x) \, dx - \int_{0}^{1} \varphi(x) \, dx + 2\varphi(0) + 2\varphi(1)$$
    \end{solution}
    }{}
\end{enumerate}
