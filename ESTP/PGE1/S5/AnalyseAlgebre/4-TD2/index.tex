...
Pour chacun des espaces \(L^p\) suivants et chacune des fonctions précédentes, déterminer si elle appartient à cet espace. Si c'est le cas, donner sa norme \(L^p\).
\[
...
\[
\int_0^\infty |e^{ix}e^{-x}| dx = \int_0^\infty e^{-x} dx = 1
\]
...
\begin{enumerate}
    ...
    \begin{solution}
        Pour $x \in ]0,1]$ fixé, on a $f_n(x) = \sqrt{n} \mathbf{1}_{[0,\frac{1}{n}]}(x) = 0$ à partir d'un certain rang (dès que $1/n < x$).
        
        \textit{Remarque : La convergence n'est pas vraie en $x=0$ car $f_n(0) = \sqrt{n} \xrightarrow{n\to\infty} +\infty$.}
    \end{solution}
    }{}
    \item Calculer $\|f_n\|_p$ pour tout $p \geq 1$ et $p=\infty$. En déduire que $(f_n)$ converge dans $L^1(]0, 1[)$ mais pas dans $L^2(]0, 1[)$, ni dans $L^\infty(]0, 1[)$.
    \ifthenelse{\boolean{showSolutions}}{
    ...
    \begin{solution}
        ...
        Pour $p = 2$, $\|f_n\|_2 = 1$ donc $\|f_n\|_2$ ne tend pas vers $0$ quand $n \to \infty$. La fonction ne converge donc pas vers la fonction nulle dans $L^2(]0,1[)$.
...
\begin{enumerate}
    ...
    \[
    S_n(f)(x) = \sum_{n \in \mathbb{Z}} c_n e^{2 i \pi n x}
    \]
    ...
    \begin{solution}
        La fonction $f$ est bornée sur l'intervalle borné $[0,1[$, donc $f \in L^2(]0,1[)$.
