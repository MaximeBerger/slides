...
\begin{enumerate}
    \item Soit $f : \mathbb{R}^+ \to \mathbb{C}$ une fonction \textbf{causale} (nulle pour $t < 0$). Montrer que :
    \[
    ...
    \item Calculer la transformée de Fourier de $g(t) = e^{-a|t|}$ pour $a > 0$ (fonction non causale).
    
    \textit{Indication : séparer l'intégrale en $t < 0$ et $t > 0$.}
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
        \begin{align*}
    ...
    \ifthenelse{\boolean{showSolutions}}{
    \begin{solution}
        \textbf{(a)} En appliquant Laplace (conditions initiales nulles) :
        \[
        (s^2 + 2s + 2)Y(s) = X(s)
        \]
        \[
        ...
        Le maximum du gain est $|H(0)| = \frac{1}{2}$, atteint en $\omega = 0$ (DC).
    \end{solution}
    }{}
\end{enumerate}
