\documentclass[../PolyS4.tex]{subfiles}
\begin{document}
...
Soit l'expérience aléatoire consistant à jeter un dé jusqu'à ce qu'un six apparaisse pour la première fois et soit $X$ la v.a. \og nombre de jets nécessaires\fg{} . 
...
\textbf{ La loi uniforme discrète}
...
\textbf{ La loi de Bernoulli et la loi binomiale $\mathcal{B}(n,p)$}
%\begin{multicols}{2}
...
$X$ prend la valeur $1$ si l'événement se produit et $0$ s'il ne se produit pas à l'issue de l'expérience. Dans ce contexte, $p$ représente la probabilité de l'événement considéré. La v.a. $X$ sera une variable de comptage lors de répétitions de l'expérience constituant le processus de Bernoulli décrit ci-après et conduisant notamment à la loi binomiale. Par convention, la réalisation de l'événement sera appelée \og succès\fg{}  et sera codée $1$, sa non-réalisation sera appelée \og échec\fg{}  et sera codée $0$.
\end{Rmq}
...
sélectionné au hasard n individus dans une grande population  on peut \og estimer\fg{}  la proportion p d'individus ayant un caractère1 donné (succès).\\
...
\textbf{ Les lois g\' eom\' etrique $\mathcal{G}(p)$ et binomiale n\' egative $\mathcal{BN}(r,p)$}

\begin{Def}\textbf{Loi géométrique}\\
Soit un processus de Bernoulli de paramètre p. La loi géométrique $\mathcal{G}(p)$, ou loi de Pascal, est la loi de la v.a. $X$ \og nombre d'échecs avant de parvenir au premier succès\fg{} . L'ensemble des valeurs possibles est $\mathbb{N}$ et la fonction de probabilité est :
...
La loi binomiale négative est une généralisation de la loi géométrique où l'on considère $X$ \og nombre d'échecs avant de parvenir au r-ième succès\fg{} . Sa fonction de probabilité est :
...
\textbf{ La loi continue uniforme $\mathcal{U}_{[a,b]}$}
...
\textbf{La loi exponentielle $\mathcal{E}(\lambda)$}
...
\textbf{ La loi de Gauss ou loi normale $\mathcal{N}(\mu,\sigma^2)$}
...
\textbf{ La loi du Khi-deux $\chi^2(n)$}
...
\item Soit $E$ l'événement : \og  chacune des 3 périodes de fonctionnement de la machine dure plus de 2 heures \fg{} . Calculer $P(E)$.
...
\begin{enumerate}
 \item Démontrer que $G(x)=_{+\infty}o\big(F(x)\big)$.
