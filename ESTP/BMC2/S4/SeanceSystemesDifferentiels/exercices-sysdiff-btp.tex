...
\begin{solution}
\textbf{a)} En développant (avec les coefficients $h_{\mathrm{ext}} = 1$, $h_{12} = 2$ du modèle) :
\[
\begin{cases}
...
On peut prendre $P = \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}$ et $D = \begin{pmatrix} -1 & 0 \\ 0 & -5 \end{pmatrix}$.
\end{solution}

  \item Effectuer le changement d'inconnue $Y = P^{-1}X$, et écrire le nouveau système obtenu avec $Y$.

\begin{solution}
\textbf{d)} Comme $X' = AX$ et $X = PY$, on a $Y' = P^{-1}X' = P^{-1}AP\,Y = DY$, d'où $y_1' = -y_1$ et $y_2' = -5y_2$.
\end{solution}

  \item Résoudre le système d'inconnue $Y$, et en déduire la solution générale du système d'inconnue~$X$.

\begin{solution}
\textbf{e)} Pour $Y$ : $y_1(t) = k_1 e^{-t}$, $y_2(t) = k_2 e^{-5t}$. D'où, avec $X = PY$,
\[
...
\end{solution}
  ...
  \item Le matin, le bureau 1 a été chauffé : $\theta_1(0) = 25\,^{\circ}\mathrm{C}$, tandis que le bureau 2 est à $\theta_2(0) = 15\,^{\circ}\mathrm{C}$ au-dessus de l'extérieur. Déterminer les constantes $C_1$ et $C_2$.

\begin{solution}
\textbf{f)} $X(0) = (25, 15)^\top$ donne $C_1 + C_2 = 25$ et $C_1 - C_2 = 15$, soit $C_1 = 20$, $C_2 = 5$.
\end{solution}
...
\begin{solution}
\textbf{h) Interprétation des modes :}
\begin{itemize}[nosep]
...
\begin{solution}
La deuxième équation est inchangée : $\theta_2' = 2\theta_1 - 3\theta_2$. D'où
...
\begin{solution}
À l'équilibre, $AX_{\mathrm{eq}} + B = 0$, soit $X_{\mathrm{eq}} = -A^{-1}B$.
On a $\det A = 5$ et
...
\begin{solution}
Avec $Y = X - X_{\mathrm{eq}}$, on a $Y' = X'$ (car $X_{\mathrm{eq}}$ est constant). D'où
...
Les volumes sont $V_A = 200\;\mathrm{m^3}$ pour le bassin~A et $V_B = 100\;\mathrm{m^3}$ pour le bassin~B. Les deux bassins sont reliés par une pompe de recirculation : le débit est $Q = 200\;\mathrm{m^3/h}$ dans chaque sens (échange d'eau de A vers B et de B vers A).

\medskip

En outre, chaque bassin est vidangé vers une station de traitement : \newline 
le bassin~A au débit $d_A = 400\;\mathrm{m^3/h}$, le bassin~B au débit $d_B = 200\;\mathrm{m^3/h}$. \newline
L'eau traitée (propre) est réinjectée de sorte que les volumes $V_A$ et $V_B$ restent constants.
...
\begin{solution}
Bilan de masse sur le bassin~A (volume $V_A = 200$~m$^3$) : entrée de polluant depuis B par la recirculation ($Q\,c_2$), sorties vers B ($Q\,c_1$) et vers la station de traitement ($d_A\,c_1$, l'eau évacuée emporte le polluant à la concentration du bassin).
\[
...
\begin{solution}
$\chi_A(\lambda) = (\lambda+3)(\lambda+4) - 2 = \lambda^2 + 7\lambda + 10 = (\lambda+2)(\lambda+5)$.
...
\end{solution}
  ...
  \item En début de journée, les concentrations mesurées sont $c_1(0) = 8\;\mathrm{mg/L}$ et $c_2(0) = 2\;\mathrm{mg/L}$. Déterminer l'évolution des concentrations dans les bassins au cours du temps.

\begin{solution}
$X(0) = (8, 2)^\top$ donne $C_1 + C_2 = 8$, $C_1 - 2C_2 = 2$, soit $C_1 = 6$, $C_2 = 2$.
\[
...
\begin{solution}
Le mode dominant est $6e^{-2t}$. Pour $c_1(t) \leq 0{,}5$ : en négligeant $e^{-5t}$, $6e^{-2t} \approx 0{,}5$ donne $t \approx \frac{1}{2}\ln 12 \approx 1{,}24$~h. Un calcul numérique plus précis (ou une vérification graphique) confirme que les deux bassins satisfont la norme vers $t \approx 1{,}3$~h, soit environ \textbf{1\,h\,15\,min}.
...
Les colonnes sont schématisées verticalement sur la figure ; les coefficients $k_1$ et $k_2$ désignent les \emph{raideurs latérales équivalentes} des trames entre niveaux (réponse élastique au cisaillement horizontal), et non les raideurs axiales des poteaux --- c'est le modèle classique de \emph{bâtiment en cisaille}.
...
\medskip
\noindent\textbf{Rappel (PFD).}
Pour un solide de masse $m$ en translation rectiligne, le principe fondamental de la dynamique s'écrit $\vec{F}_{\mathrm{res}} = m\,\vec{a}$. En projection sur l'axe horizontal du séisme, pour chaque plancher de déplacement $x(t)$ :
\[
  m\,\ddot{x}(t) = \sum F_h,
\]
où $\sum F_h$ est la somme algébrique des forces horizontales exercées sur ce plancher (rappel élastique des colonnes, etc.). Pour un ressort de raideur $k$ reliant deux points de déplacements $x$ et $y$, la force de rappel qu'il exerce sur le point $x$ vaut $k\,(y-x)$ (loi de Hooke).
...
\begin{solution}
PFD sur le plancher 1 : la force de rappel des colonnes du RDC est $-k_1 x_1$, celle des colonnes de l'étage est $k_2(x_2 - x_1)$. Donc :
\[
...
\begin{solution}
$\chi_\Omega(\lambda) = (\lambda - 5)(\lambda - 2) - 4 = \lambda^2 - 7\lambda + 6 = (\lambda - 1)(\lambda - 6)$.
...
\begin{solution}
Pulsations : $\omega_1 = 1\;\mathrm{rad/s}$, $\omega_2 = \sqrt{6} \approx 2{,}45\;\mathrm{rad/s}$.
...
\begin{solution}
Solution générale : on cherche $X(t) = \phi(t) U$ avec $\ddot{\phi} = -\omega^2 \phi$, soit $\phi(t) = A\cos(\omega t + \varphi)$.
\[
